...
{
    public abstract class PortalWebTestBase : AbpAspNetCoreIntegratedTestBase<Startup>
    {
        protected static readonly Lazy<string> ContentRootFolder;

        static PortalWebTestBase()
        {
        ...
        protected override IWebHostBuilder CreateWebHostBuilder()
        {
            return base
                ...
                .UseSetting(WebHostDefaults.ApplicationKey, typeof(PortalWebMvcModule).Assembly.FullName);
        }
        ...
        {
            using (var context = IocManager.Resolve<PortalDbContext>())
            {
                action(context);
                context.SaveChanges();
            }
        ...
        protected async Task UsingDbContextAsync(Func<PortalDbContext, Task> action)
        {
            using (var context = IocManager.Resolve<PortalDbContext>())
            {
        ...
        protected async Task<T> UsingDbContextAsync<T>(Func<PortalDbContext, Task<T>> func)
        {
            T result;
